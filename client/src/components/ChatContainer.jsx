    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className=" flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6">
        {messagesDummyData.map((msg, index) => (
          <div
            key={index}
            className={` flex items-end gap-2 text-gray-500
            ${msg.senderId !== "680f50f3cd28382ecf9" && " flex-row-reverse"}`}
          >
            {msg.image ? (
              <img
                src={msg.image}
      ...
      >
        <div className=' flex-1 flex items-center bg-gray-100/12
         px-3 rounded-full'>
          ...
          <label htmlFor="image">
            <img src={assets.gallery_icon} alt="gallary" className='w-5 mr-2 
             cursor-pointer' />
          </label>
        </div>
        <img src={assets.send_button} alt="send button" 
          className='w-7 cursor-pointer m-1'
        />
      </div>
    </div>
  ) : (
